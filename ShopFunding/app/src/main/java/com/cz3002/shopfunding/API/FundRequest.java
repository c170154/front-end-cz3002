...
public class FundRequest {
    public static boolean createRequest(
            Context context, String productName, String description, String url, float goal, String jwt_token
    ) {
        ...
        JSONObject json_response = requestManager.postRequest(ENDPOINTS.FUND_REQUEST, payload, jwt_token);
        return json_response != null && !json_response.isNull("name");
    }

    public static ArrayList<FundingRequest> fetchFundRequests(Context context, String jwt_token) {
        RequestManager requestManager = RequestManager.getInstance(context.getApplicationContext());
        JSONArray json_response = requestManager.getRequest_JSONArray(ENDPOINTS.FUND_REQUEST, jwt_token);
        if (json_response != null) {
            ...
            try {
                for (int i=0; i < json_response.length(); i++) {
                    ...
                    fundRequests.add(new FundingRequest(productName, description, url, goal));
                }
            } catch (JSONException e) {
                e.printStackTrace();
            }
